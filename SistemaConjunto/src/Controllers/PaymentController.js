...
function payments (req, res) {
    ...
    if(!body.torre || !body.apartamento){
        ...
        paymentService.getPayments(body, res)
    }
...
function pay (req, res) {
    ...
    if(!body.torre || !body.apartamento ){
        ...
        paymentService.pay(body, res)
    }
};

function certificate (req, res) {
    const { body } = req;
    if(!body.torre || !body.apartamento){
        res.status(400).send({
            status: "FAILED",
        ...
        paymentService.certificate(body, res)
    }
};
...
module.exports = {payments, bill, certificate, pay};